...
def format_base(number: int) -> str:
    return str(number).zfill(BASE_WIDTH)


def base_range(first: int = DEFAULT_FIRST, last: int = DEFAULT_LAST) -> List[str]:
    if first < 1 or last < first:
        raise ValueError(f"invalid range {first}..{last}")
    return [format_base(n) for n in range(first, last + 1)]


def normalize(raw: str) -> Optional[str]:
    """Canonicalise an id, or return None if it is not one.
...
def split_id(issue_id: str) -> Tuple[int, str]:
    ...
    match = ID_RE.match(issue_id.upper())
    if not match:
...
def in_range(issue_id: str, first: int = DEFAULT_FIRST, last: int = DEFAULT_LAST) -> bool:
    ...
    four-digit tokens (years, counts, element ids) out of the work list.
    """
    try:
